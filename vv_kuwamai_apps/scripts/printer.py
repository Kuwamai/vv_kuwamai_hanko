...
class ThermalPrinter(object):
    ...
    #def __init__(self, heatTime=1, heatInterval=255, heatingDots=255, serialport=SERIALPORT):
    def __init__(self, heatTime=100, heatInterval=250, heatingDots=1, serialport=SERIALPORT):
        self.printer = Serial(serialport, self.BAUDRATE, timeout=self.TIMEOUT)
        #self.printer = Serial(serialport, self.BAUDRATE, timeout=self.TIMEOUT, rtscts=True)
        #self.printer.write(chr(29))      # GS - command
        #self.printer.write(chr(97))      # a
        #self.printer.write(chr(1<<5))    # RTS -> Allow
    ...
    def set_param(self):
        ...
        #self.printer.write(self._ESC) # ESC - command
        #self.printer.write(chr(64)) # @   - initialize
        self.printer.write(27) # ESC - command
        self.printer.write(55) # 7   - print settings
        self.printer.write(1)  # Heating dots (20=balance of darkness vs no jams) default = 20
        self.printer.write(255) # heatTime Library default = 255 (max)
        self.printer.write(255) # Heat interval (500 uS = slower, but darker) default = 250
        ...
        # D4..D0 of n is used to set the printing density. Density is 50% + 5% * n(D4-D0) printing density.
        # D7..D5 of n is used to set the printing break time. Break time is n(D7-D5)*250us.
        printDensity = 10 # 120% (? can go higher, text is darker but fuzzy)
        printBreakTime = 15 # 500 uS
        self.printer.write(18)
        self.printer.write(35)
        self.printer.write((printDensity << 4) | printBreakTime)
...
if __name__ == '__main__':
    ...
    p = ThermalPrinter(heatingDots=0, heatTime=255, heatInterval=255, serialport=serialport)
    ...
    p.print_text("--- start print ---")
    from PIL import Image
    #i = Image.open("example-lammas.png")
    #i = Image.open("test_image.jpg")
    i = Image.open("./Astronaut_scaled.jpg").convert('L')
    #i = Image.open("gene_image1.png")
    data = list(i.getdata())
    w, h = i.size
